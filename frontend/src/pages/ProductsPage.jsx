  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-wrap gap-4 mb-8">
        ...
        <Button variant={filter === 'porcelanato' ? 'default' : 'outline'} onClick={() => setFilter('porcelanato')}>Porcelanato</Button>
        <Button variant={filter === 'madeira' ? 'default' : 'outline'} onClick={() => setFilter('madeira')}>Madeira</Button>
          ...
          {filteredProducts.map(product => (
            <Card key={product.id} className="hover:shadow-lg transition-shadow">
              <CardHeader className="relative">
                {/* Linha de avaliação e favorito alinhados */}
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-sm text-gray-500">
                      {product.average_rating ? `${product.average_rating.toFixed(1).replace('.0', '')}/5` : "Sem notas"}
                    </span>
                  </div>
